...
pub fn bresenham_function(v0: Vec2, v1: Vec2, p: Vec2) -> f32
{
    0.0
}
...
fn main() {
    ...
    let mut redTriangle: bool = false;
    ...
    while window.is_open() && !window.is_key_down(Key::Escape) {
        for i in buffer.iter_mut() {
            *i = 0; //screen clear
            ...
            if !wireFrameRend
            {
                m0 = edge_function(triangle[0], triangle[1], coords);
                m1 = edge_function(triangle[1], triangle[2], coords);
                m2 = edge_function(triangle[2], triangle[0], coords);
            } else {
                //do bresenham here
            ...
            if wireFrameRend {
                // render with bresenham algorithm
            }
